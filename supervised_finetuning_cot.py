...
def get_args():
    ...
    parser.add_argument("--dataset_name", type=str, default="./data/cot/")
...
def create_datasets(tokenizer, args):
    train_json_path = os.path.join(args.dataset_name, "train_instruction.json")
    train_data = load_dataset("json", data_files=train_json_path, split="train")
    train_data = train_data.shuffle(seed=args.seed)

    val_json_path = os.path.join(args.dataset_name, "val_instruction.json")
...
def main(args):
    if "llama" in args.base_model.lower():
        tokenizer = LlamaTokenizer.from_pretrained(args.base_model, local_files_only=True)
        print('tokenizer load from local done')
